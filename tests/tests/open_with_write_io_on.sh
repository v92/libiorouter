# Test if file is deleted from caching directory during IO routing OFF.
#!/bin/bash
ROOTDIR=${PWD%*/*/*}
...
echo LIBIOR_IO=off LIBIOR_REWRITEDIR=$TESTDIR/nfsmnt LIBIOR_CACHEDIR=$TESTDIR/run LD_PRELOAD=$ROOTDIR/libiorouter.so strace -s 256 $TESTDIR/tests/open_with_write $TESTDIR/nfsmnt/test.php > $TESTDIR/tests/open_with_write.runstr
...
assertFalse "$TESTDIR/nfsmnt/test.php MUST NOT exist in $TESTDIR/run" "[ -f $TESTDIR/run/$TESTIDR/test.php ]"
}
